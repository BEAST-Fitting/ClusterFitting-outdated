...
pro fit_cluster_prob_sparse,star_filebase,run_tag=run_tag,mod_tag=mod_tag,field_eps=field_eps, $
...
;        fullprob = dblarr(size_oneprob[1],size_oneprob[2],size_oneprob[3],size_oneprob[4],n_files)
        ...
        n_sparse = long(0.10*n_files*n_elements(oneprob))
        fullprob_sparse = dblarr(n_sparse)
        fullprob_sparse_indxs = lonarr(n_sparse)
        fullprob_sparse_startstop = lonarr(2,n_files)
        cur_sparse = 0
    endif
    ...
    oneprob /= total(oneprob)
;    fullprob[*,*,*,*,i] = oneprob

    gindxs = where(oneprob GT 1d-50,n_gindxs)
;    print,n_gindxs,double(n_gindxs)/double(n_elements(oneprob))

    fullprob_sparse[cur_sparse:cur_sparse+n_gindxs-1] = oneprob[gindxs]
    fullprob_sparse_indxs[cur_sparse:cur_sparse+n_gindxs-1] = gindxs
    fullprob_sparse_startstop[*,i] = [cur_sparse,cur_sparse+n_gindxs-1]
    cur_sparse += n_gindxs
endfor
print,cur_sparse,n_sparse
...
;                        tprob = total(tprob_full)
                        ...
                        ; compute the total probabilty using the sparse form
                        ss = fullprob_sparse_startstop[*,c]
                        tprob_full_sparse = fullprob_sparse[ss[0]:ss[1]]*p_gamma_theta[fullprob_sparse_indxs[ss[0]:ss[1]]]
                        tprob_sparse = total(tprob_full_sparse)
;                        print,c,tprob,tprob_sparse,tprob/tprob_sparse,ss[0],ss[1]
                        
                        ; code to penalize non-fitting stars when there is no field in the model
                        if ((field_eps LE 0.0) AND (tprob_sparse LT 1e-20)) then tprob_sparse = 1e-20

                        theta_prob[i,j,k,l,m] += alog(tprob_sparse)
